...
// Run inference on our passed image.
...
NSString* RunInferenceOnImage(UIImage* image) {
    ...
    // Read the label list
    NSString* labels_path = FilePathForResourceName(@"lens_graph_label_strings", @"txt");
...
//    std::vector<tensorflow::uint8> image_data = LoadImageFromFile([image_path UTF8String], &image_width, &image_height, &image_channels);
    ...
    // convert CGImage into data
    std::vector<tensorflow::uint8> image_data = LoadImage(image.CGImage, &image_width, &image_height, &image_channels);
    ...
    for (const auto& result : top_results) {
        ...
        if (index < label_strings.size()) {
            // just for safety: theoretically, the output is under 1000 unless there is some numerical issues leading to a wrong prediction.
