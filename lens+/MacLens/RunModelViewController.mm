...
-(NSString*)analyseImage:(NSImage*)image {
...
NSString* RunInferenceOnImage(NSImage* image) {
    ...
    NSString* network_path = FilePathForResourceName(@"lens_graph", @"pb");
    ...
    // Read the label list
    NSString* labels_path = FilePathForResourceName(@"lens_label_string", @"txt");
...
//    NSString* image_path = FilePathForResourceName(@"ISIC_0000254", @"jpg");
...
//    std::vector<tensorflow::uint8> image_data = LoadImageFromFile([image_path UTF8String], &image_width, &image_height, &image_channels);
    ...
    CGRect frameXYZMUTHAFUKA = CGRectMake(0, 0, image.size.width, image.size.height);
    
    CGImage *imgC = [image CGImageForProposedRect:&frameXYZMUTHAFUKA context:nil hints:nil];
    
    std::vector<tensorflow::uint8> image_data = LoadImage(imgC, &image_width, &image_height, &image_channels);
    ...
    result = [NSString stringWithFormat: @"%s", predictions.c_str()];
